...
export default function Cart() {
  ...
  const handleCheckout = () => {
    ...
    toast.success('Order placed! Check Dashboard for status.')
  ...
  if (items.length === 0) return (
    <main className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="text-center">
        ...
        <Link to="/" className="bg-primary text-white px-6 py-3 rounded-xl font-semibold hover:bg-primary-dark transition">Browse Listings</Link>
      </div>
    </main>
  )
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-10">
      <h1 className="font-heading font-bold text-2xl text-gray-900 mb-6">Your Cart ({items.length})</h1>
      <div className="space-y-4 mb-6">
        {items.map(item => (
          <div key={item.id} className="bg-white border border-gray-100 rounded-2xl p-4 flex gap-4 shadow-sm">
            ...
            <button onClick={() => removeFromCart(item.id)} aria-label={`Remove ${item.title}`}
